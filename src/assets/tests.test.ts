...
import { StorageSerializers, customStorageEventName } from '@vueuse/core'
...
import { useSetup, mount } from './utils'
...
describe('useReactiveStorage', () => {
  ...
  it('export module', () => {
    expect(useReactiveStorage).toBeDefined()
    expect(StorageSerializers).toBeDefined()
  })

  it('string', async () => {
    const vm = useSetup(() => {
      const ref = useReactiveStorage(KEY, 'a', storage)
  ...
  it('number', async () => {
    ...
    const store = useReactiveStorage(KEY, 1, storage)
  ...
  it('boolean', async () => {
    const store = useReactiveStorage(KEY, true, storage)
  ...
  it('null string', () => {
    ...
    const store = useReactiveStorage(KEY, null, storage)
  ...
  it('null value', () => {
    ...
    const store = useReactiveStorage(KEY, null)
    const storedValue = storage.getItem(KEY)
  ...
  it('undefined value', () => {
    ...
    const store = useReactiveStorage(KEY, undefined, storage)
    const storedValue = storage.getItem(KEY)
  ...
  it('date', async () => {
    ...
    const store = useReactiveStorage(KEY, new Date('2000-01-02T00:00:00.000Z'), storage)
  ...
  it('object', async () => {
    ...
    const store = useReactiveStorage(KEY, {
      name: 'a',
      data: 123,
    }, storage)
  ...
  it('map', async () => {
    const store = useReactiveStorage(KEY, new Map<number, string | number>([
      [1, 'a'],
      [2, 2],
    ]), storage)
  ...
  it('set', async () => {
    ...
    const store = useReactiveStorage(KEY, new Set<string | number>([1, '2']), storage)
  ...
  it('pass ref as initialValue', async () => {
    ...
    const state = useReactiveStorage(KEY, init, storage)
  ...
  it('custom serializer', async () => {
    ...
    const vm = useSetup(() => {
      const ref = useReactiveStorage(KEY, 0, storage, { serializer: { read: JSON.parse, write: JSON.stringify } })
  ...
  it('mergeDefaults option', async () => {
    ...
    const basicRef = useReactiveStorage(KEY, 1, storage, { mergeDefaults: true })
    expect(basicRef.value).toBe(0)

    // object
    storage.setItem(KEY, JSON.stringify({ a: 1 }))
    const objectRef = useReactiveStorage(KEY, { a: 2, b: 3 }, storage, { mergeDefaults: true })
    expect(objectRef.value).toEqual({ a: 1, b: 3 })

    // array
    storage.setItem(KEY, JSON.stringify([1]))
    const arrayRef = useReactiveStorage(KEY, [2], storage, { mergeDefaults: true })
    expect(arrayRef.value).toEqual([1])

    // custom function
    storage.setItem(KEY, JSON.stringify([{ a: 1 }]))
    const customRef = useReactiveStorage(KEY, [{ a: 3 }], storage, { mergeDefaults: (value, initial) => [...initial, ...value] })
    expect(customRef.value).toEqual([{ a: 3 }, { a: 1 }])

    // custom function 2
    storage.setItem(KEY, '1')
    const customRef2 = useReactiveStorage(KEY, 2, storage, { mergeDefaults: (value, initial) => value + initial })
    expect(customRef2.value).toEqual(3)
  })

  it('use storage value if present', async () => {
    storageState.set(KEY, 'true')
    expect(useReactiveStorage(KEY, false, storage).value).toBe(true)

    storageState.set(KEY, '0')
    expect(useReactiveStorage(KEY, 1, storage).value).toBe(0)

    storageState.set(KEY, 0)
    expect(useReactiveStorage(KEY, 1, storage).value).toBe(0)

    storageState.set(KEY, JSON.stringify({}))
    expect(useReactiveStorage(KEY, { a: 1 }, storage).value).toEqual({})

    storageState.set(KEY, '')
    expect(useReactiveStorage(KEY, null, storage).value).toBe('')

    storageState.set(KEY, 'a')
    expect(useReactiveStorage(KEY, 'b', storage).value).toBe('a')

    storageState.set(KEY, JSON.stringify([]))
    expect(useReactiveStorage(KEY, new Map([[1, 2]]), storage).value).toEqual(new Map([]))

    storageState.set(KEY, JSON.stringify([]))
    expect(useReactiveStorage(KEY, new Set([1, 2]), storage).value).toEqual(new Set([]))
  })

  it('support shallow ref', async () => {
    const data = useReactiveStorage(KEY, 0, storage, { shallow: true })
  ...
  it('watch storage event', async () => {
    const data = useReactiveStorage(KEY, 0, localStorage)
    expect(data.value).toBe(0)
  ...
  it('emits custom storage events on change', async () => {
    const eventFn = vi.fn()
    const data0 = useReactiveStorage(KEY, 0, storage)
    const data1 = useReactiveStorage(KEY, 0, storage)
  ...
  it('initOnMounted', async () => {
    ...
    const vm = mount(defineComponent({
      setup() {
        const basicRef = useReactiveStorage(KEY, '', storage, { initOnMounted: true })
  ...
  it.each([
    ...
    const basicRef = useReactiveStorage(KEY, () => value, storage)
    expect(basicRef.value).toEqual(value)
    storage.removeItem(KEY)
    const objectRef = useReactiveStorage(KEY, value, storage)
    expect(objectRef.value).toEqual(value)
  })

  it('syncs properly within the same document', async () => {
    const state1 = useReactiveStorage(KEY, 0, storage)
    const state2 = useReactiveStorage(KEY, 0, storage)
  ...
  it('syncs properly within the same document (custom storages)', async () => {
    ...
    const state1 = useReactiveStorage(KEY, 0, customStorage)
    const state2 = useReactiveStorage(KEY, 0, customStorage)
